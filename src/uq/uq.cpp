...
//         }
        ...
        if (maxPolyOrder > 0) {
          hermitePoly[1][j] = x ;
          if (maxPolyOrder > 1) {
            for (unsigned ipoly = 2; ipoly <= maxPolyOrder; ipoly++) {
              hermitePoly[ipoly][j] = x * hermitePoly[ipoly - 1][j] - (ipoly - 1) * hermitePoly[ipoly - 2][j];
            }
            for (unsigned ipoly = 2; ipoly <= maxPolyOrder; ipoly++) {
              unsigned factorial = 1;
              unsigned ipolyFactorial = ipoly;
              while (ipolyFactorial >= 1) {
                factorial *= ipolyFactorial;
                ipolyFactorial -= 1;
              }
              hermitePoly[ipoly][j] /= sqrt (factorial);
            }
          }
        }
...
//         }
        ...
        if (maxPolyOrder > 0) {
          ...
          if (maxPolyOrder > 1) {
            for (unsigned ipoly = 2; ipoly <= maxPolyOrder; ipoly++) {
              double scaling = 1. / ipoly;
              legendrePoly[ipoly][j] = scaling * ( (2. * (ipoly - 1) + 1.) * x * legendrePoly[ipoly - 1][j] - (ipoly - 1) * legendrePoly[ipoly - 2][j]);
            }
            for (unsigned ipoly = 0; ipoly <= maxPolyOrder; ipoly++) {
              legendrePoly[ipoly][j] *= sqrt (2. * ipoly + 1.);
            }
          }
        }
...
//       }
      ...
      if (pIndex > 0) {
        _polynomialHistogram[1][j] = x ;
        if (pIndex > 1) {
          for (unsigned ipoly = 2; ipoly <= pIndex; ipoly++) {
            _polynomialHistogram[ipoly][j] = x * _polynomialHistogram[ipoly - 1][j] - (ipoly - 1) * _polynomialHistogram[ipoly - 2][j];
          }
          for (unsigned ipoly = 2; ipoly <= pIndex; ipoly++) {
            unsigned factorial = 1;
            unsigned ipolyFactorial = ipoly;
            ...
            _polynomialHistogram[ipoly][j] /= sqrt (factorial);
          }
...
//       }
      ...
      if (pIndex > 0) {
        ...
        if (pIndex > 1) {
          for (unsigned ipoly = 2; ipoly <= pIndex; ipoly++) {
            double scaling = 1. / ipoly;
            _polynomialHistogram[ipoly][j] = scaling * ( (2. * (ipoly - 1) + 1.) * x * _polynomialHistogram[ipoly - 1][j] - (ipoly - 1) * _polynomialHistogram[ipoly - 2][j]);
          }
          for (unsigned ipoly = 0; ipoly <= pIndex; ipoly++) {
            _polynomialHistogram[ipoly][j] *= sqrt (2. * ipoly + 1.);
          }
        }
      }
