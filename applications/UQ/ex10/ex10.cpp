...
//
void GetHistogramAndKDE (std::vector< std::vector < double > > & sgmQoIStandardized, std::vector< std::vector <double > > & sgmQoIStandardizedFinest, MultiLevelSolution& mlSolHisto, MultiLevelSolution& mlSolHistoFinest);
//
void PlotGCandEDExpansion();

void GetKDEIntegral (MultiLevelSolution& mlSolHisto);

void GetAverageL2Error (std::vector< std::vector <double > > & sgmQoIStandardized, MultiLevelSolution& mlSolHisto, MultiLevelSolution& mlSolHistoFinest);

void GetL2Error (std::vector< std::vector <double > > & sgmQoIStandardized, MultiLevelSolution& mlSolHisto, MultiLevelSolution& mlSolHistoFinest, const bool &analytic);
...
std::cout<< "xSampleMin = " << xSampleMin << " , " << "xSampleMax = " << xSampleMax << " , " << "ySampleMin = " << ySampleMin << " , " << "ySampleMax = " << ySampleMax << std::endl; 
  ...
  GetHistogramAndKDE (sgmQoIStandardized, sgmQoIStandardizedFinest, mlSolHisto, mlSolHistoFinest);
...
void GetQoIStandardizedSamples (std::vector< double >& alphas, std::vector< std::vector <double > > & sgmQoIStandardized, std::vector< std::vector <double > > &                               sgmQoIStandardizedFinest, const unsigned & dimCoarseBox) {
  ...
  //FOR NON STD GAUSSIAN SAMPLING
  boost::mt19937 rng3;
  boost::normal_distribution<> nd3 (0., 2.);
  boost::variate_generator < boost::mt19937&,
        boost::normal_distribution<> > var_nor3 (rng3, nd3);
  ...
  for (unsigned m = 0; m < sgmQoIStandardized.size(); m++) {
    ...
    for (unsigned idim = 0; idim < dimCoarseBox; idim++) {
      ...
      std::cout <<  samplePoints[0] << " , " << samplePoints[1] << " , " << sgmQoIStandardized[m][idim] << std::endl;

      if (samplePoints[0] > xSampleMax) xSampleMax = samplePoints[0];
      if (samplePoints[0] < xSampleMin) xSampleMin = samplePoints[0];
      if (samplePoints[1] > ySampleMax) ySampleMax = samplePoints[1];
      if (samplePoints[1] < ySampleMin) ySampleMin = samplePoints[1];
...
// // //
//       if (sgmQoIStandardized[m][0] > ymax) ymax = sgmQoIStandardized[m][0];
//       if (sgmQoIStandardized[m][0] < ymin) ymin = sgmQoIStandardized[m][0];
...
void GetHistogramAndKDE (std::vector< std::vector <double > > & sgmQoIStandardized, std::vector< std::vector <double > > & sgmQoIStandardizedFinest, MultiLevelSolution& mlSolHisto, MultiLevelSolution& mlSolHistoFinest) {
  ...
  char name[10];
  sprintf (name, "HISTO");
  double solIndexHISTO = mlSolHisto.GetIndex (name);   // get the position of "Ti" in the sol object
  sprintf (name, "PROPOSED");
  double solIndexKDE = mlSolHisto.GetIndex (name);   // get the position of "Ti" in the sol object

  unsigned solTypeHISTO = mlSolHisto.GetSolutionType (solIndexHISTO);
  unsigned solTypeKDE = mlSolHisto.GetSolutionType (solIndexKDE);
  ...
  for (unsigned m = 0; m < numberOfSamples; m++) {
    ...
    if (dim == 1) {
      ...
      unsigned firstDof = msh->_dofOffset[solTypeKDE][iproc];
      unsigned lastDof = msh->_dofOffset[solTypeKDE][iproc + 1] - 1;
      ...
      for (int iel = sol->GetMesh()->_elementOffset[iproc]; iel < sol->GetMesh()->_elementOffset[iproc + 1]; iel ++) {
        ...
        if (sgmQoIStandardized[m][0] > xLeft && sgmQoIStandardized[m][0] <= xRight) {
          ...
          for (unsigned inode = 0; inode < nDofsKDE; inode++) {
            ...
            double scalingFac = (globalDof == lastDof || globalDof == firstDof) ? 1. : 1.;
            double KDEvalue = scalingFac * phi[inode] / (numberOfSamples * dx);   //note: numberOfSamples * h is an approximation of the area under the histogram, aka the integral
      ...
      Marker marker (sgmQoIStandardized[m], 0., VOLUME, mlSolHisto.GetLevel (level), 2, true);
...
//BEGIN computation of histo finest
    ...
    double solIndexHISTOF = mlSolHistoFinest.GetIndex (name);   // get the position of "Ti" in the sol object
    ...
    for (unsigned m = 0; m < numberOfSamplesFinest; m++) {
      ...
      if (dim == 1) {
        ...
        //BEGIN write finest histogram solution
        Marker marker2 (sgmQoIStandardizedFinest[m], 0., VOLUME, mlSolHistoFinest.GetLevel (level), 2, true);
...
void GetAverageL2Error (std::vector< std::vector <double > > & sgmQoIStandardized, MultiLevelSolution& mlSolHisto, MultiLevelSolution& mlSolHistoFinest) {
  ...
  char name[10];
  sprintf (name, "PROPOSED");
  double solIndexKDE = mlSolHisto.GetIndex (name);
  unsigned solTypeKDE = mlSolHisto.GetSolutionType (solIndexKDE);

  sprintf (name, "HISTO");
  double solIndexHISTOError = mlSolHisto.GetIndex (name);

  sprintf (name, "HISTO_F");
  double solIndexHISTO = mlSolHistoFinest.GetIndex (name);
...
//
//           aL2ELocal += (solKDESample - laplaceDist) * (solKDESample - laplaceDist);
...
//         double diffPHI1 = 0.5 * (1. + erf (2.75 / sqrt (2))) - 0.5 * (1. + erf (-2.75 / sqrt (2)));
//         double stdGaussian1 = (exp (- sgmQoIStandardized[m][0] * sgmQoIStandardized[m][0] * 0.125) / sqrt (8. * PI)) / diffPHI1;
//
//         double diffPHI2 = 0.5 * (1. + erf (5.5 / sqrt (2))) - 0.5 * (1. + erf (-5.5 / sqrt (2)));
//         double stdGaussian2 = (exp (- sgmQoIStandardized[m][1] * sgmQoIStandardized[m][1] * 0.5) / sqrt (2. * PI)) / diffPHI2;
//
//         double jointPDF = stdGaussian1 * stdGaussian2;
//
...
//         if (histoErr) aL2ELocalHisto += (solHISTOSampleError - jointPDF) * (solHISTOSampleError - jointPDF);
      ...
      if (histoFinest) {
        ...
        //BEGIN evaluate the finest histogram at the sample
        Marker marker2 (sgmQoIStandardized[m], 0., VOLUME, mlSolHistoFinest.GetLevel (level), 2, true);
        unsigned iel2 = marker2.GetMarkerElement();
    ...
    for (unsigned ig = 0; ig < msh->_finiteElement[ielGeom][solTypeKDE]->GetGaussPointNumber(); ig++) {
      ...
      if (analytic) {
        ...
        if (dim == 1) {
          ...
          Marker marker (x_gss, 0., VOLUME, mlSolHistoFinest.GetLevel (level), 2, true);
